...
static void
set_window_gravity(Window win, int gravity)
{
   u_long wamask;
   XSetWindowAttributes wa;
   ...
   XChangeWindowAttributes(dpy, win, wamask, &wa);
}


/* if I want to combine movement+resize w/ frame change, then this has to get
 * back the original code! I drop it now!*/
static void
move_resize_frame(Lisp_Window *win, int x,int y,int w,int h)
{
   ...
   */
   ...
   if (win->reparented && bigger) /* mmc: why ....if it's bigger,
                                   * it's nice to move and indeed see all the contents,
...

   {
      ...
      /* I know the new & old position, new position of each FP,
       * and the frame-size & movement delta. */
       ...
       */
       ...
       /* old: i should calculate the displacement of all fps and if
        * it's 0,0 -> make it static geome. */
      ...
      /* mmc: question: is the final dimensions the one requested by the window?
       * (perhaps in a hint)  */
      if (win->reparented)
   ...
   if (!win->reparented || !bigger) /* not bigger? why is that relevant?
                                   * it is just to complement
                                   * the case solved several lines above.
                                   * We need set_frame_shapes always */
      set_frame_shapes (win, TRUE);
